...
class FragmentCriarReuniaoOportunidade : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        edtxt_tipo_reuniao.text = Editable.Factory.getInstance().newEditable(getString(R.string.str_reuniao_sobre_oportunidade))
        ...
        if (GlobalVariables.criarReuniaoOportunidade) {
            txt_titulo_janela.text = getString(R.string.str_criar_reuniao_oportunidade)
            btn_criar_reuniao_entrevista.text = getString(R.string.str_criar_reuniao_oportunidade)
            chbx_notificar_participantes.visibility = View.VISIBLE
        } else {
            txt_titulo_janela.text = getString(R.string.str_editar_reuniao_oportunidade)
            btn_criar_reuniao_entrevista.text = getString(R.string.str_editar_reuniao_oportunidade)
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            if (globalVariables.checkForInternet(requireContext())) {
                                postUtilizadorCliente(context, GlobalVariables.detalhesNumCliente, NReunioes)
                            } else {
                                Toast.makeText(context, "Sem conexão à Internet.", Toast.LENGTH_LONG).show()
                            }
                        } else {
    ...
    private fun postUtilizadorCliente(context: Context, numUtilizador: Int, numReuniao: Int) {
        data class Post(val NUsuario: Int, val NReunioes: Int)
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                println("Falha na solicitação de agregação do cliente à reunião: ${e.message}")

                try {
                    requireActivity().runOnUiThread {
                        val builder = AlertDialog.Builder(context)
                        builder.setTitle("Erro")
                        builder.setMessage("Falha na solicitação de agregação do cliente à reunião: ${e.message}")
                        builder.setIcon(android.R.drawable.ic_dialog_alert)
                        builder.setPositiveButton("OK") { dialog, _ ->
                            dialog.dismiss()
                        }
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            msgPosts += "<b>Agregação do cliente à reunião:</b><br>O cliente \'${GlobalVariables.detalhesNomeCliente}\' foi adicionado à lista de participantes da reunião com sucesso.<br>"
                            val builder = AlertDialog.Builder(context)
                            builder.setTitle("Aviso")
                            builder.setMessage(Html.fromHtml(msgPosts))
                            builder.setIcon(R.drawable.ic_information)
                            ...
                            val message = jsonObject.getString("message")
                            msgPosts += "<b>Agregação do cliente à reunião:</b><br><b>Falha na agregação do cliente à reunião:</b> $message<br>"
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            val builder = AlertDialog.Builder(context)
                            builder.setTitle("Aviso")
                            builder.setMessage("A reunião do negócio foi atualizada com sucesso.")
                            builder.setIcon(R.drawable.ic_information)
                            builder.setPositiveButton("OK") { dialog, _ ->
                                dialog.dismiss()
                                val fragment = parentFragmentManager.beginTransaction()
