...
class FragmentCriarReuniaoEntrevista : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        edtxt_tipo_reuniao.text = Editable.Factory.getInstance().newEditable(getString(R.string.str_reuniao_sobre_entrevista))
        ...
        if (GlobalVariables.criarReuniaoEntrevista) {
            txt_titulo_janela.text = getString(R.string.str_criar_reuniao_entrevista)
            btn_criar_reuniao_entrevista.text = getString(R.string.str_criar_reuniao_entrevista)
            chbx_notificar_participantes.visibility = View.VISIBLE
        } else {
            txt_titulo_janela.text = getString(R.string.str_editar_reuniao_entrevista)
            btn_criar_reuniao_entrevista.text = getString(R.string.str_editar_reuniao_entrevista)
    ...
    private fun putCandidatura(context: Context) {
        data class Post(val Estagio: String)

        val client = OkHttpClient.Builder()
            .addInterceptor(GlobalVariables.DefaultContentTypeInterceptor())
            .build()

        val post = Post("Entrevista")
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                println("Falha na solicitação de edição da candidatura: ${e.message}")

                try {
                    requireActivity().runOnUiThread {
                        val builder = AlertDialog.Builder(context)
                        builder.setTitle("Erro")
                        builder.setMessage("Falha na solicitação de edição da candidatura: ${e.message}")
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            msgPosts += "<b>Edição da candidatura:</b><br>O estágio da candidatura foi atualizado com sucesso.<br>"
                            if (globalVariables.checkForInternet(context)) {
                                postEntrevista(context)
                            } else {
                                Toast.makeText(context, "Sem conexão à Internet.", Toast.LENGTH_LONG).show()
                            }
                        } else {
                            val message = jsonObject.getString("message")
                            msgPosts += "<b>Edição da candidatura:</b><br><b>Falha na edição da candidatura:</b> $message<br>"
    ...
    private fun postEntrevista(context: Context) {
        data class Post(val NCandidatura: Int, val Descricao: String)

        val client = OkHttpClient.Builder()
            .addInterceptor(GlobalVariables.DefaultContentTypeInterceptor())
            .build()

        val post = Post(GlobalVariables.detalhesNumCandidatura, strDescricaoReuniao)
        val requestBody = Gson().toJson(post).toRequestBody("application/json".toMediaTypeOrNull())
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                println("Falha na solicitação de criação da entrevista: ${e.message}")

                try {
                    requireActivity().runOnUiThread {
                        val builder = AlertDialog.Builder(context)
                        builder.setTitle("Erro")
                        builder.setMessage("Falha na solicitação de criação da entrevista: ${e.message}")
                        builder.setIcon(android.R.drawable.ic_dialog_alert)
                        builder.setPositiveButton("OK") { dialog, _ ->
                            dialog.dismiss()
                        }
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            msgPosts += "<b>Criação da entrevista:</b><br>A entrevista do candidato \'${GlobalVariables.detalhesNomeUsuarioCandidatura}\' foi criada com sucesso.<br>"
                            if (globalVariables.checkForInternet(context)) {
                                postReuniao(
                                    context,
                            ...
                            val message = jsonObject.getString("message")
                            msgPosts += "<b>Criação da entrevista:</b><br><b>Falha na edição da candidatura:</b> $message<br>"
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            if (globalVariables.checkForInternet(context)) {
                                postUtilizadorCandidatoReuniao(
                                    context,
                                    GlobalVariables.detalhesNumUsuarioCandidatura,
                                    NReunioes
                                )
                            } else {
                                Toast.makeText(context, "Sem conexão à Internet.", Toast.LENGTH_LONG).show()
    ...
    private fun postUtilizadorCandidatoReuniao(context: Context, numUtilizador: Int, numReuniao: Int) {
        data class Post(val NUsuario: Int, val NReunioes: Int)
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                println("Falha na solicitação de agregação do candidato à reunião: ${e.message}")

                try {
                    requireActivity().runOnUiThread {
                        val builder = AlertDialog.Builder(context)
                        builder.setTitle("Erro")
                        builder.setMessage("Falha na solicitação de agregação do candidato à reunião: ${e.message}")
                        builder.setIcon(android.R.drawable.ic_dialog_alert)
                        builder.setPositiveButton("OK") { dialog, _ ->
                            dialog.dismiss()
                        }
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            msgPosts += "<b>Agregação do candidato à reunião:</b><br>O candidato \'${GlobalVariables.detalhesNomeUsuarioCandidatura}\' foi adicionado à lista de participantes da reunião com sucesso.<br>"
                            val builder = AlertDialog.Builder(context)
                            builder.setTitle("Aviso")
                            builder.setMessage(Html.fromHtml(msgPosts))
                            builder.setIcon(R.drawable.ic_information)
                            ...
                            val message = jsonObject.getString("message")
                            msgPosts += "<b>Agregação do candidato à reunião:</b><br><b>Falha na agregação do candidato à reunião:</b> $message<br>"
                ...
                try {
                    requireActivity().runOnUiThread {
                        ...
                        if (success) {
                            ...
                            val builder = AlertDialog.Builder(context)
                            builder.setTitle("Aviso")
                            builder.setMessage("A reunião da entrevista foi atualizada com sucesso.")
                            builder.setIcon(R.drawable.ic_information)
                            builder.setPositiveButton("OK") { dialog, _ ->
                                dialog.dismiss()
                                val fragment = parentFragmentManager.beginTransaction()
